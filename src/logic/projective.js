...
pj.check = function(a){
    console.log("pj.check")
    return a*2
}

pj.setConstants = function(lfx, lfy, lcx, lcy){
    // console.log("setConstants")
    fx = lfx
    fy = lfy
    cx = lcx
    cy = lcy
...
pj.checkIfVisibleAxis = function(curCameraPosition, curAngleX, curAngleY, curAngleZ, vidWidth, vidHeight){
	var offset = 100
...
pj.getCameraPositionWithSquarePoints = function(angleX, angleY, angleZ, cameraSquarePoints){
	...
	let norm = pj.getHorizontalPlaneNorm(-angleX, -angleY, angleZ)
	// console.log("norm1 ", norm1)
	// let norm =  [0, Math.cos(-angleX), Math.sin(-angleX)]  
	// console.log("norm ", norm)
	let squarePointsXy1 = pj.pointsUvToXy1(cameraSquarePoints)
	// console.log(squarePointsXy1)
	var p1 = pj.getCrossPoint(squarePointsXy1[0], norm, 1)
	var p2 = pj.getCrossPoint(squarePointsXy1[1], norm, 1)
	var p3 = pj.getCrossPoint(squarePointsXy1[2], norm, 1)
	var p4 = pj.getCrossPoint(squarePointsXy1[3], norm, 1)
	
	let dist = (pj.distBetweenPoints(p1, p2) + pj.distBetweenPoints(p2, p3) + pj.distBetweenPoints(p3, p4) + pj.distBetweenPoints(p4, p1)) / 4

	let d = -side*2/dist

	p1 = pj.getCrossPoint(squarePointsXy1[0], norm, d)
	p2 = pj.getCrossPoint(squarePointsXy1[1], norm, d)
	p3 = pj.getCrossPoint(squarePointsXy1[2], norm, d)
	p4 = pj.getCrossPoint(squarePointsXy1[3], norm, d)
...
pj.getCameraPositionWithZ = function(angleX, angleY, angleZ, cameraPoint, point3d, cameraZ){
    console.log(fx, fy, cx, cy)
	var xy1 = [ (cameraPoint[0] - cx) / fx, (cameraPoint[1] - cy) / fy, 1]
    // console.log(xy1)
	let curCameraPos = pj.getCameraPositionFromXyzAndAngles(angleX, angleY, angleZ, xy1, point3d).cameraPos
    // console.log(curCameraPos)
	var t =  (point3d[2] - cameraZ)  / (point3d[2] - curCameraPos[2]);
    // console.log(t)
	var xyz = [ xy1[0] * t, xy1[1] * t, t ]
    console.log(xyz) //[-0.12515164268788453, -1.1263647841909605, 15.26850040792191] [-0.11475409836065573, -0.7540983606557378, 10]
	console.log(point3d) //    [0, 0, 0]
    return pj.getCameraPositionFromXyzAndAngles(angleX, angleY, angleZ, xyz, point3d)
}
...
pj.getCameraPositionFromXyzAndAngles = function(angleX, angleY, angleZ, xyz, point3d){
	...
	let point1 = pj.rotatePointY(xyz1, -angleY)
	point1 = pj.rotatePointX(point1, -angleX)
	point1 = pj.rotatePointZ(point1, -angleZ)
	point1[2] = -point1[2]

	let point = pj.rotatePointY(xyz, -angleY)
	point = pj.rotatePointX(point, -angleX)
	point = pj.rotatePointZ(point, -angleZ)
	point[2] = -point[2]
	// console.log(point)
	let cameraPos = [point3d[0] - point[0], point3d[1] - point[1], point3d[2] - point[2]]
	let cameraLookAt = [cameraPos[0] + point1[0], cameraPos[1] + point1[1], cameraPos[2] + point1[2]]
